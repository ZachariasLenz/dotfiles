...
local function has_lsp()
   return vim.tbl_contains(
      vim.tbl_flatten(vim.tbl_values(filetypes.lsp)),
      vim.bo.filetype
   )
end
...
local function get_lsp_clients()
   ...
   for _, client in ipairs(clients) do
      if client.name:lower() ~= "diagnosticls" then
         return client.name
      end
   end
   return "diagnosticls"
...
gls.left[4] = {
   LeftSeparator = {
      condition = has_lsp,
      highlight = { colors.nord1, colors.nord1 },
      provider = function()
         return " "
      end,
   },
...
gls.left[10] = {
   LeftSpace = {
      highlight = { colors.nord3, colors.nord3 },
      provider = function()
         return " "
      end,
   },
